...
def gen_recipes():
    ...
    # Rods
    for metal, data in MATERIALS.items():
        if "rod" in data["forms"] and "ingot" in data["forms"]:
    ...
    for metal, data in MATERIALS.items():
        if "plate" in data["forms"] and "ingot" in data["forms"]:
            ...
            write_json(f"{DATA_DIR}/recipes/{metal}_plate.json", {
                "type": "minecraft:crafting_shapeless",
                "ingredients": [{"item": f"{MOD_ID}:{metal}_ingot"}, {"item": f"{MOD_ID}:iron_hammer"}],
